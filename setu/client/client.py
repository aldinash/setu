...
from setu.client.tensor_handles import (
    CachedTensorHandle,
    TensorReadHandle,
    TensorWriteHandle,
)
...
class Client:
    ...
    def __init__(self, endpoint: str) -> None:
        ...
        self._tensor_handle_cache: Dict[uuid.UUID, CachedTensorHandle] = {}
    ...
    def get_cached_tensor_handle(
        self, shard_id: uuid.UUID
    ) -> Optional[CachedTensorHandle]:
        """Look up a cached tensor handle by shard ID.

        Args:
            shard_id: UUID of the shard to look up

        Returns:
            CachedTensorHandle if cached, None otherwise
        """
        return self._tensor_handle_cache.get(shard_id)

    def set_cached_tensor_handle(
        self, shard_id: uuid.UUID, cached: CachedTensorHandle
    ) -> None:
        """Store a tensor handle in the cache.

        Args:
            shard_id: UUID of the shard
            cached: CachedTensorHandle to store
        """
        self._tensor_handle_cache[shard_id] = cached

    def get_tensor_handle(self, shard_ref: TensorShardRef):
        """Get the IPC handle for a tensor shard from the NodeAgent."""
        return self._client.get_tensor_handle(shard_ref)