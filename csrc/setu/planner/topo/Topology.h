...
struct Link {
    explicit Link(
        float latency_us_param,
        float bandwidth_gbps_param,
    ...
    [[nodiscard]] std::string ToString() const {
        return std::format("Link(latency={}us, bandwidth={}GB/s, tag={})", bandwidth_gbps,latency_us,tag);
    }
...
struct Path {
    explicit Path(
        std::vector<Participant> hops_param,
    ...
    float TransferTimeUsPipelined(std::size_t bytes) const {
        ...
        return total_latency_us + static_cast<float>(bytes) / (bottleneck_bandwidth_gbps * 1e3f);
    }

    [[nodiscard]] std::string ToString() const {
      return std::format("Path(hops={},links={},total_latency={}us,bottleneck_bandwidth={}GB/s)", hops, links, total_latency_us, bottleneck_bandwidth_gbps);
    }
...
private:
    static float ComputeTotalLatency(const std::vector<Link>& links) {
        return std::accumulate(links.begin(), links.end(), 0.0f,
...
void AddBidirectionalLink(const Participant& src, const Participant& dst, Link link);

std::vector<Link> QueryLinks(const Participant& src, const Participant& dst) const;

std::optional<Link> QueryBestLink(const Participant& src, const Participant& dst, std::size_t bytes) const;

std::optional<Path> ShortestPath(const Participant& src, const Participant& dst, std::function<float(const Link&)> cost_fn) const;

private:
    std::unordered_map<Participant, std::vector<std::pair<Participant, Link>>> adj_;
};
