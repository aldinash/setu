...
void Topology::AddLink(const Participant& src, const Participant& dst, Link link) {
...
void Topology::AddBidirectionalLink(const Participant& src, const Participant& dst, Link link) {
    AddLink(src,dst,link);
    AddLink(dst,src,link);
}

std::vector<Link> Topology::QueryLinks(const Participant& src, const Participant& dst) const {
...
std::optional<Link> Topology::QueryBestLink(const Participant& src, const Participant& dst, std::size_t bytes) const {
...
std::optional<Path> Topology::ShortestPath(const Participant& src, const Participant& dst, std::function<float(const Link&)> cost_fn) const {
    if (src == dst) {
        return Path({src}, {});
    }
