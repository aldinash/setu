...
def _register_and_get_handle(endpoint: str, tensor_name: str, dims_spec):
    ...
    return response.tensor_ipc_spec
...
def test_multiple_shards_same_client_get_handles(infrastructure):
    ...
    for shard_ref, (tensor_name, dim_sizes) in zip(registered_refs, tensor_configs):
        response = client.get_tensor_handle(shard_ref)
        assert response is not None, f"Failed to get handle for {tensor_name}"

        spec_dict = response.tensor_ipc_spec.to_dict()
...
def test_distributed_tensor_allocation(multi_node_infrastructure):
    ...
    client_0 = Client()
    client_0.connect(infra["client_endpoint_0"])
    response_0 = client_0.get_tensor_handle(shard_ref_0)
    client_0.disconnect()

    client_1 = Client()
    client_1.connect(infra["client_endpoint_1"])
    response_1 = client_1.get_tensor_handle(shard_ref_1)
    client_1.disconnect()

    assert response_0 is not None, "NodeAgent 0 should have allocated tensor"
    assert response_1 is not None, "NodeAgent 1 should have allocated tensor"

    assert response_0.tensor_ipc_spec.to_dict()["tensor_size"] == [512, 768]
    assert response_1.tensor_ipc_spec.to_dict()["tensor_size"] == [512, 768]
